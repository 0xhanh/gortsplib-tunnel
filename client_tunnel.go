...
- https://stackoverflow.com/questions/2408233/how-to-force-client-to-switch-rtp-transport-from-udp-to-tcp/3536969#3536969
*/
...
func (c *ClientTunnel) canonicalAddr(u *url.URL) string {
	...
	if c.tunnelOverPort > 0 {
		port = fmt.Sprintf("%d", c.tunnelOverPort)
	} else if port == "" {
		if u.Scheme == "rtsp" {
			port = "554"
		} else { // rtsps
...
func (s clientState) String() string {
	...
	case clientStageGet:
		return "get"
	case clientStagePost:
		return "post"
	case clientStageDesc:
		return "desc"
...
// Client is a RTSP tunnel client.
...
func (c *ClientTunnel) Start(scheme string, host string) error {
	if c.tunnelOverPort == 0 {
	...
	c.chGet = make(chan getReq)
	c.chPost = make(chan postReq)
...
func (c *ClientTunnel) Wait() error {
	<-c.done
	return c.closeError
}

func (c *ClientTunnel) run() {
	defer close(c.done)
...
func (c *ClientTunnel) runInner() error {
	for {
		select {
		// tunnel:
		case req := <-c.chGet:
			res, err := c.doGet(req.url)
...
func (c *ClientTunnel) waitResponse(requestCseqStr string) (*base.Response, error) {
...
func (c *ClientTunnel) handleServerRequest(req *base.Request) error {
...
func (c *ClientTunnel) doClose() {
	...
	// tunnel:
	if c.controlNconn != nil {
		c.controlNconn.Close()
	}
	c.controlNconn = nil
	c.controlConn = nil
}

func (c *ClientTunnel) reset() {
	c.doClose()
...
func (c *ClientTunnel) checkState(allowed map[clientState]struct{}) error {
...
func (c *ClientTunnel) startReadRoutines() {
...
func (c *ClientTunnel) stopReadRoutines() {
	if c.reader != nil {
...
func (c *ClientTunnel) startWriter() {
	c.writer.start()
}

func (c *ClientTunnel) stopWriter() {
	c.writer.stop()
}

func (c *ClientTunnel) connOpen() error {
	if c.nconn != nil {
		return nil
	}
	...
	nconn, err := c.DialContext(dialCtx, "tcp", c.canonicalAddr(c.connURL))
	if err != nil {
		return err
	}
...
// tunnel: make an another connection for control channel
func (c *ClientTunnel) controlConnOpen() error {
	...
	c.controlNconn = nconn
	bc := bytecounter.New(c.controlNconn, c.BytesReceived, c.BytesSent)
	c.controlConn = conn.NewConn(bc)
	// no need reader/response for this connn
	// c.reader = newClientReader(c)

	return nil
}

func (c *ClientTunnel) do(req *base.Request, skipResponse bool) (*base.Response, error) {
	if c.state == clientStagePost && req.Method != base.Options {
	...
	if res.StatusCode == base.StatusUnauthorized && req.URL.User != nil && c.sender == nil {
		...
		// if Method == Get -> reset & make new conn
		// todo
	}
...
// tunnel:
func (c *ClientTunnel) writeRequest(req *base.Request) (err error) {
	// see: https://www.happytimesoft.com/knowledge/rtsp-over-http.html

	nconn := &c.controlNconn
	conn := c.controlConn
	if req.Method == base.Get { // GET using data socket
		nconn = &c.nconn
		conn = c.conn
	}

	(*nconn).SetWriteDeadline(time.Now().Add(c.WriteTimeout))
	err = conn.WriteTunnelRequest(req)

	return
}

func (c *ClientTunnel) atLeastOneUDPPacketHasBeenReceived() bool {
...
func (c *ClientTunnel) isInUDPTimeout() bool {
	now := c.timeNow()
	for _, ct := range c.medias {
...
func (c *ClientTunnel) doKeepAlive() error {
...
func (c *ClientTunnel) GET(u *url.URL) (*base.Response, error) {
	...
	select {
	case c.chGet <- getReq{url: u, res: cres}:
		res := <-cres
...
func (c *ClientTunnel) doGet(u *url.URL) (*base.Response, error) {
	...
	res, err := c.do(&base.Request{
		...
		Header: base.Header{
			"x-sessioncookie": base.HeaderValue{c.sessionCookie},
			"Accept":          base.HeaderValue{"application/x-rtsp-tunnelled"},
			"Pragma":          base.HeaderValue{"no-cache"},
...
func (c *ClientTunnel) doPost(u *url.URL) (*base.Response, error) {
	...
	res, err := c.do(&base.Request{
		...
		Header: base.Header{
			"x-sessioncookie": base.HeaderValue{c.sessionCookie},
			"Accept":          base.HeaderValue{"application/x-rtsp-tunnelled"},
			"Content-Type":    base.HeaderValue{"application/x-rtsp-tunnelled"},
			"Pragma":          base.HeaderValue{"no-cache"},
			"Cache-Control":   base.HeaderValue{"no-cache"},
			"Content-Length":  base.HeaderValue{"32767"},
			"Expires":         base.HeaderValue{"Sun, 9 Jan 1972 00:00:00 GMT"},
		},
	}, true)
...
func (c *ClientTunnel) doDescribe(u *url.URL) (*description.Session, *base.Response, error) {
	...
	if res.StatusCode != base.StatusOK {
		...
		// 	// tunnel can not redirect
		// }

		// Axis 490 Account Block??
...
func (c *ClientTunnel) doSetup(
	...
	case TransportUDP, TransportUDPMulticast:
		if thRes.Protocol == headers.TransportProtocolTCP {
			...
			if c.effectiveTransport == nil &&
				c.Transport == nil {
				c.baseURL = baseURL
				return nil, fmt.Errorf("tunnel has not supported switch transport automatically")
			}
	...
	if c.state == clientStateInitial || c.state == clientStageDesc {
...
func (c *ClientTunnel) isChannelPairInUse(channel int) bool {
	for _, cm := range c.medias {
...
func (c *ClientTunnel) findFreeChannelPair() int {
...
func (c *ClientTunnel) doPlay(ra *headers.Range) (*base.Response, error) {
	err := c.checkState(map[clientState]struct{}{
		clientStatePrePlay: {},
	})
	if err != nil {
...
func (c *ClientTunnel) Play(ra *headers.Range) (*base.Response, error) {
	cres := make(chan clientRes)
	select {
...
func (c *ClientTunnel) doRecord() (*base.Response, error) {
	err := c.checkState(map[clientState]struct{}{
		clientStatePreRecord: {},
	})
	if err != nil {
...
func (c *ClientTunnel) Record() (*base.Response, error) {
	cres := make(chan clientRes)
	select {
...
func (c *ClientTunnel) doPause() (*base.Response, error) {
	err := c.checkState(map[clientState]struct{}{
		clientStatePlay:   {},
		clientStateRecord: {},
	})
...
func (c *ClientTunnel) Pause() (*base.Response, error) {
	cres := make(chan clientRes)
	select {
...
func (c *ClientTunnel) Seek(ra *headers.Range) (*base.Response, error) {
	_, err := c.Pause()
	if err != nil {
		return nil, err
	}
...
func (c *ClientTunnel) OnPacketRTPAny(cb OnPacketRTPAnyFunc) {
	for _, cm := range c.medias {
		cmedia := cm.media
...
func (c *ClientTunnel) OnPacketRTCPAny(cb OnPacketRTCPAnyFunc) {
	for _, cm := range c.medias {
		cmedia := cm.media
...
func (c *ClientTunnel) OnPacketRTP(medi *description.Media, forma format.Format, cb OnPacketRTPFunc) {
	cm := c.medias[medi]
...
func (c *ClientTunnel) OnPacketRTCP(medi *description.Media, cb OnPacketRTCPFunc) {
	cm := c.medias[medi]
	cm.onPacketRTCP = cb
}

// WritePacketRTP writes a RTP packet to the server.
func (c *ClientTunnel) WritePacketRTP(medi *description.Media, pkt *rtp.Packet) error {
...
func (c *ClientTunnel) WritePacketRTPWithNTP(medi *description.Media, pkt *rtp.Packet, ntp time.Time) error {
...
func (c *ClientTunnel) WritePacketRTCP(medi *description.Media, pkt rtcp.Packet) error {
...
func (c *ClientTunnel) PacketPTS(medi *description.Media, pkt *rtp.Packet) (time.Duration, bool) {
	cm := c.medias[medi]
	ct := cm.formats[pkt.PayloadType]
...
func (c *ClientTunnel) PacketNTP(medi *description.Media, pkt *rtp.Packet) (time.Time, bool) {
	cm := c.medias[medi]
	ct := cm.formats[pkt.PayloadType]
	return ct.rtcpReceiver.PacketNTP(pkt.Timestamp)
}

func (c *ClientTunnel) readResponse(res *base.Response) {
	c.chReadResponse <- res
}

func (c *ClientTunnel) readRequest(req *base.Request) {
	c.chReadRequest <- req
}

func (c *ClientTunnel) readError(err error) {
	c.chReadError <- err
}
